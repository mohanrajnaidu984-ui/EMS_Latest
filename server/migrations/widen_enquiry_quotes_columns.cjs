...
/** Widen EnquiryQuotes string columns that are too small for real quote payloads. */
const ALTERS = [
    ['ToPhone', 255],
    ['ToEmail', 255],
    ['PreparedByEmail', 255],
    ...
    for (const [col, chars] of ALTERS) {
        const check = await new sql.Request()
            ...
            .query(`
                ...
                WHERE c.object_id = OBJECT_ID('dbo.EnquiryQuotes') AND c.name = @col
            `);
        ...
        const sqlText = `ALTER TABLE dbo.EnquiryQuotes ALTER COLUMN [${col}] NVARCHAR(${chars}) NULL`;
